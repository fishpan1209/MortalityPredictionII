...
dead = FILTER eventswithmort BY mortality::label==1;
deadevents = FOREACH dead GENERATE $0 AS patientid, $1 AS eventid, $3 AS value, $6 AS label, (DaysBetween($5,$2)-30) AS time_difference;  
...
alive = FILTER eventswithmort BY mortality::label IS NULL;
alive = FOREACH alive GENERATE $0 AS patientid, $1 AS eventid, $2 AS timestamp, $3 AS value, 0 AS label;
index_alive = FOREACH (GROUP alive by patientid) GENERATE group AS patientid,MAX(alive.timestamp) AS index_date;
alive_index = JOIN alive BY patientid, index_alive by patientid;
aliveevents = FOREACH alive_index GENERATE $0 AS patientid, $1 AS eventid, $3 AS value, $4 AS label, DaysBetween(index_alive::index_date, alive::timestamp) AS time_difference;
...
filtered_dead = FILTER deadevents BY time_difference <= 2000 AND time_difference>=0 AND eventid is not null AND value is not null AND label is not null AND time_difference is not null;
filtered_alive = FILTER aliveevents BY time_difference <= 2000 AND time_difference>=0 AND eventid is not null AND value is not null AND label is not null AND time_difference is not null;
...
filtered_agg = GROUP filtered BY (patientid, eventid);
featureswithid = FOREACH filtered_agg GENERATE FLATTEN(group) AS (patientid, eventid), COUNT(filtered.eventid) AS featurevalue;
--TEST-3
...
feature_rank = GROUP featureswithid BY eventid;
feature_rank = RANK feature_rank BY group ASC;
all_features = FOREACH feature_rank GENERATE ($0-1) AS idx, $1 AS eventid;
...
features_j = JOIN featureswithid BY eventid LEFT OUTER, all_features BY eventid;
features = FOREACH features_j GENERATE $0 AS patientid, $3 AS idx, $2 as featurevalue;
--TEST-4
...
maxvalues = FOREACH features_group GENERATE group AS idx, MAX(features.featurevalue) AS maxvalues;
...
features = FOREACH normalized GENERATE $0 AS patientid, $1 AS idx, $2 * 1.0 /$4 AS normalizedfeaturevalue;
--TEST-5
features = ORDER features BY patientid, idx;
